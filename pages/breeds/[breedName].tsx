...
const renderBars = (breed: Breed) => (
  <>
    ...
    <ExpandableDescription
      emoji="🌟"
      title="적응력"
      introText="이 항목은 강아지가 다양한 환경에 얼마나 잘 적응하는지를 평가합니다. 이 점수는 강아지가 새로운 환경이나 변화된 상황에 대처하는 능력을 나타냅니다. 적응력이 높을수록 스트레스 없이 다양한 상황에 쉽게 적응할 수 있습니다."
      level={breed.adaptabilityLevel}
      descriptions={[
        '1점 - 환경 변화에 매우 민감하며, 새로운 환경에 적응하기 어려워합니다. 종종 변화로 인해 스트레스를 받을 수 있으며, 적응하는 데 오랜 시간이 필요할 수 있습니다.',
        '2점 - 환경 변화에 민감하며, 적응하는 데 시간이 걸립니다. 새로운 환경에 익숙해지기 위해 노력해야 하며, 스트레스를 받을 가능성이 큽니다.',
        '3점 - 보통으로 적응하며, 약간의 시간이 필요합니다. 새로운 환경에 익숙해지는 데 다소 시간이 걸리지만, 큰 문제 없이 적응할 수 있습니다.',
        '4점 - 환경 변화에 잘 적응하며 큰 어려움 없이 적응합니다. 새로운 환경에서 스트레스를 거의 받지 않으며, 비교적 쉽게 적응하는 모습을 보입니다.',
        '5점 - 환경 변화에 매우 잘 적응하며, 빠르게 문제없이 적응합니다. 새로운 장소나 상황에서도 스트레스 없이 활동하며, 다양한 상황에서도 안정적입니다.'
      ]}
    />
    ...
    <ExpandableDescription
      emoji="❤️"
      title="가족과의 애정"
      introText="이 항목은 강아지가 가족과 얼마나 깊은 유대감을 형성하고 애정을 나누는지를 평가합니다. 애정이 많은 강아지는 가족과의 지속적인 상호작용을 필요로 하고, 사랑과 관심을 자주 표현하는 반면, 애정이 적은 강아지는 독립적인 성향을 가지며 가족과의 상호작용을 덜 필요로 합니다. 이 평가를 통해 가족과의 관계 형성 방식과 강아지의 성향을 이해할 수 있습니다."
    ...
    <ExpandableDescription
      emoji="🐕"
      title="다른 개와의 친화력"
      introText="이 항목은 강아지가 다른 반려견과 얼마나 잘 어울리고, 사회적 관계를 형성할 수 있는지를 평가합니다. 친화력이 높은 강아지는 다른 반려견들과 쉽게 어울리며, 사회적인 활동을 즐깁니다. 반면, 친화력이 낮은 강아지는 독립적이거나 공격적인 성향을 보일 수 있으며, 상호작용을 피하려는 경향이 있습니다. 이를 통해 반려견의 성격과 사회성을 고려하여 적합한 환경을 마련할 수 있습니다."
    ...
    <ExpandableDescription
      emoji="👶"
      title="아이와의 친화력"
      introText="이 항목은 강아지가 어린 자녀와 얼마나 잘 어울리고, 상호작용하는지를 평가합니다. 어린 자녀와의 상호작용에서 강아지가 보이는 성향은 가족 생활에 중요한 영향을 미칩니다. 친화력이 높은 강아지는 자녀와 자연스럽게 어울리며, 보호자 없이도 안전하게 놀 수 있습니다. 반면, 친화력이 낮은 강아지는 예민하거나 공격적인 성향을 보일 수 있으므로 주의가 필요합니다. 이를 통해 어린 자녀가 있는 가정에서 적합한 강아지를 선택할 수 있습니다."
    ...
    <ExpandableDescription
      emoji="🐾"
      title="타인에 대한 개방성"
      introText="이 항목은 강아지가 낯선 사람에게 얼마나 친절하고 개방적인지를 평가합니다. 타인에 대한 개방성이 높은 강아지는 쉽게 사회적 관계를 형성하며, 사람들과의 만남에서 적극적으로 상호작용하려는 경향이 있습니다. 반면, 개방성이 낮은 강아지는 경계심이 강하고, 새로운 사람들과 친해지기 어려울 수 있습니다. 이를 통해 강아지의 성향에 맞는 사회적 환경을 제공할 수 있습니다."
    ...
    <ExpandableDescription
      emoji="🛡️"
      title="보호 본능"
      introText="이 항목은 강아지가 가족과 집을 보호하려는 본능이 얼마나 강한지를 평가합니다. 보호 본능이 강한 강아지는 경계심이 높아 가족과 집을 보호하려는 행동을 자주 보입니다. 반면, 보호 본능이 약한 강아지는 낯선 사람이나 환경에 큰 경계심을 보이지 않으며, 가족을 보호하려는 역할보다는 더 온순한 성향을 보일 수 있습니다. 이를 통해 보호 본능이 강한 반려견이 필요한지 여부를 판단할 수 있습니다."
    ...
    <ExpandableDescription
      emoji="⚡"
      title="에너지 수준"
      introText="이 항목은 강아지가 얼마나 활발하고 에너지가 넘치는지를 평가합니다. 에너지 수준이 높은 강아지는 하루 종일 활동을 필요로 하며, 많은 운동과 놀이가 필수적입니다. 반면, 에너지 수준이 낮은 강아지는 주로 조용하게 시간을 보내며, 간단한 산책이나 짧은 놀이로도 충분히 만족할 수 있습니다. 이를 통해 강아지가 필요로 하는 활동량을 파악하고, 가정 환경에 적합한 반려견을 선택할 수 있습니다."
    ...
    <ExpandableDescription
      emoji="🎮"
      title="장난기"
      introText="이 항목은 강아지가 얼마나 장난스럽고 놀기 좋아하는지를 평가합니다. 장난기가 많은 강아지는 보호자와의 놀이 시간을 매우 중요하게 생각하며, 지속적으로 놀이를 원할 수 있습니다. 반면, 장난기가 적은 강아지는 더 차분하고 조용한 성향을 보일 수 있습니다. 이 평가를 통해 강아지와의 놀이 스타일과 강아지가 필요로 하는 활동량을 이해하고, 보호자의 라이프스타일에 맞는 반려견을 선택하는 데 도움이 됩니다."
    ...
    <ExpandableDescription
      emoji="🧠"
      title="정신적 자극 필요도"
      introText="이 항목은 강아지가 얼마나 자주 정신적 자극과 훈련이 필요한지를 평가합니다. 정신적 자극이 많이 필요한 강아지는 다양한 놀이와 학습 활동을 통해 머리를 쓰는 것을 좋아하며, 자극이 부족할 경우 쉽게 지루해질 수 있습니다. 반면, 자극이 적게 필요한 강아지는 간단한 활동만으로도 만족할 수 있으며, 높은 수준의 문제 해결 능력을 요구하지 않습니다. 이 평가를 통해 강아지가 필요로 하는 정신적 자극의 양을 파악하고, 적절한 환경을 제공할 수 있습니다."
    ...
    <ExpandableDescription
      emoji="🎓"
      title="훈련 가능성"
      introText="이 항목은 강아지가 훈련을 얼마나 잘 따르고 학습 능력이 높은지를 평가합니다. 훈련 가능성이 높은 강아지는 새로운 명령을 빠르게 이해하고 반복적인 훈련을 즐기며, 보호자와의 상호작용을 통해 빠르게 학습할 수 있습니다. 반면, 훈련 가능성이 낮은 강아지는 고집이 세거나 쉽게 집중하지 못해, 훈련에 시간이 오래 걸릴 수 있습니다. 이를 통해 강아지의 학습 능력과 훈련 접근 방식을 계획하는 데 도움이 됩니다."
    ...
    <ExpandableDescription
      emoji="🪮"
      title="털 빠짐 정도"
      introText="이 항목은 강아지가 얼마나 자주 털을 빠뜨리는지를 평가합니다. 털 빠짐이 적은 강아지는 관리가 비교적 쉬우며, 주기적인 빗질만으로도 충분한 반면, 털이 많이 빠지는 강아지는 더 빈번한 청소와 그루밍이 필요할 수 있습니다. 이를 통해 보호자는 강아지의 털 관리와 주거 환경에 적합한 털 빠짐 수준을 고려할 수 있습니다."
    ...
    <ExpandableDescription
      emoji="🧼"
      title="그루밍 필요도"
      introText="이 항목은 강아지가 얼마나 자주 그루밍이 필요한지를 평가합니다. 그루밍 필요도가 낮은 강아지는 관리가 쉬우며, 털이 자주 엉키지 않고 깨끗함을 유지하기 어렵지 않습니다. 반면, 그루밍 필요도가 높은 강아지는 자주 빗질하고 손질해야 하며, 그렇지 않으면 털이 엉키거나 위생 문제가 생길 수 있습니다. 이를 통해 보호자는 강아지의 털 관리에 소요되는 시간과 노력을 파악할 수 있습니다."
    ...
    <ExpandableDescription
      emoji="🗣️"
      title="짖는 수준"
      introText="이 항목은 강아지가 얼마나 자주 짖는지를 평가합니다. 짖는 수준이 낮은 강아지는 매우 조용하고, 필요할 때만 짖는 반면, 짖는 수준이 높은 강아지는 자주 다양한 이유로 짖으며, 때로는 통제하기 어려울 수 있습니다. 이를 통해 보호자는 강아지의 짖는 성향을 이해하고, 주거 환경과의 적합성을 판단할 수 있습니다."
    ...
    <ExpandableDescription
      emoji="💧"
      title="침 흘림 수준"
      introText="이 항목은 강아지가 얼마나 자주 침을 흘리는지를 평가합니다. 침 흘림이 적은 강아지는 관리가 쉬우며, 일상 생활에서 침으로 인한 불편함이 거의 없습니다. 반면, 침 흘림이 많은 강아지는 자주 관리가 필요하고, 침이 가구나 바닥에 자주 묻을 수 있으므로 청결 유지가 중요합니다. 이를 통해 보호자는 강아지의 침 흘림 성향을 파악하고, 그에 맞는 관리 방법을 계획할 수 있습니다."
